...
async def orchestrate(config: OrchestratorConfig):
    ...
    while True:
        ...
        if config.trace_export is not None and config.trace_export.enabled:
            export_rollout_traces(
                rollouts=train_rollouts,
                step=progress.step,
                output_dir=config.trace_export.output_dir or config.output_dir,
                include_segments=config.trace_export.include_segments,
                include_metrics=config.trace_export.include_metrics,
            )
