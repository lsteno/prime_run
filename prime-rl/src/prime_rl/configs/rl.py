...
class SharedTensorBoardConfig(BaseConfig):
    """Configures shared TensorBoard settings."""

    enabled: Annotated[bool | None, Field(description="Whether to enable TensorBoard event logging.")] = None

    log_dir: Annotated[
        Path | None,
        Field(description="Optional base directory for TensorBoard logs. Trainer/orchestrator subdirectories are created automatically."),
    ] = None

    flush_secs: Annotated[int | None, Field(ge=1, description="TensorBoard flush cadence in seconds.")] = None
...
class RLConfig(BaseConfig):
    ...
    tensorboard: Annotated[
        SharedTensorBoardConfig | None,
        Field(
            description="Shared TensorBoard configs. If None, TensorBoard settings must be configured per component."
        ),
    ] = None
    ...
    def auto_setup_tensorboard(self):
        ...
        if self.tensorboard is not None:
            if not self.trainer.tensorboard:
                self.trainer.tensorboard = TensorBoardConfig()
            if not self.orchestrator.tensorboard:
                self.orchestrator.tensorboard = TensorBoardConfig()

            if self.tensorboard.enabled is not None:
                self.trainer.tensorboard.enabled = self.tensorboard.enabled
                self.orchestrator.tensorboard.enabled = self.tensorboard.enabled

            if self.tensorboard.flush_secs is not None:
                self.trainer.tensorboard.flush_secs = self.tensorboard.flush_secs
                self.orchestrator.tensorboard.flush_secs = self.tensorboard.flush_secs

            if self.tensorboard.log_dir is not None:
                self.trainer.tensorboard.log_dir = self.tensorboard.log_dir / "trainer"
                self.orchestrator.tensorboard.log_dir = self.tensorboard.log_dir / "orchestrator"
