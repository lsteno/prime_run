...
def build_config(base: str, *, rank: int, alpha: int, lr: str, run_id: str) -> str:
    ...
    text = replace_or_insert_line_after_once(
        text,
        line_pattern=r"^subcall_batch_max_workers = \d+$",
        needle=f"max_batched_subcalls = {MAX_BATCHED_SUBCALLS}\n",
        line=f"subcall_batch_max_workers = {SUBCALL_BATCH_MAX_WORKERS}",
    )
    text = replace_line(
        text,
